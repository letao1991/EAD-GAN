...
def sample_image(real_imgs, trans_img_affine,  n_cols, batches_done):
    ...
    zeros = np.zeros((n_cols *7, 1))
    c_varied = np.tile(np.linspace(-1, 1, n_cols)[:, np.newaxis], (7,1))

    #print ()
    c1 = Variable(FloatTensor(np.concatenate((c_varied, zeros, zeros, zeros), -1)))
    c2 = Variable(FloatTensor(np.concatenate((zeros, c_varied, zeros, zeros), -1)))
    c3 = Variable(FloatTensor(np.concatenate((zeros, zeros, c_varied, zeros), -1)))
    c4 = Variable(FloatTensor(np.concatenate((zeros, zeros, zeros, c_varied), -1)))
    c5 = Variable(FloatTensor(np.concatenate((c_varied, zeros, zeros, zeros), -1)))
    c6 = Variable(FloatTensor(np.concatenate((c_varied, zeros, zeros, zeros), -1)))
    c7 = Variable(FloatTensor(np.concatenate((c_varied, zeros, zeros, zeros), -1)))
...
for epoch in range(opt.n_epochs):
    for i, (img) in enumerate(dataloader):
        ...
        align_img = trans_2D(img, inv_align_matrix[:,0:2])
        ...
        trans_matrix = get_matrix_D(code_input[:,:4])
        ...
        d_real = discriminator(trans_img_affine)
        ...
        # train adv g together with info
        ...
        trans_cat, trans_cont = encoder(trans_img_affine)


        # get affine loss

        relative_affine = affine_regularzier(align_cont, trans_cont)
        affine_loss = continuous_loss(relative_affine, code_input_original)
        ...
        info_affine_color_loss = info_loss + affine_loss + g_loss + relative_cat_loss

        optimizer_info.zero_grad()
        info_affine_color_loss.backward()
        optimizer_info.step()

        #print ("code_input", code_input[0])
        ...
        if batches_done % 100 == 0:
            print(
            "[Epoch %d/%d] [Batch %d/%d] [D loss: %f] [G loss: %f] [info cat loss: %f] [info cont loss: %f] [affine loss: %f] [relative_cat_loss: %f] "
            % (epoch, opt.n_epochs, i, len(dataloader),  d_loss.item(), g_loss.item(), cat_loss.item(), cont_loss.item(),
             affine_loss.item(), relative_cat_loss.item())
            )

            print ("trans_img_affine max", torch.max(trans_img_affine))
            print ("gen_imgs max", torch.max(gen_imgs))
            print ("trans_img_affine min", torch.min(trans_img_affine))
            print ("gen_imgs min", torch.min(gen_imgs))


        if batches_done % (opt.sample_interval *2) == 0:
            sample_image(align_img[0:100], trans_img_affine[0:100], n_cols=10, batches_done=batches_done)

        if batches_done % (opt.sample_interval * 500 ) == 0:
