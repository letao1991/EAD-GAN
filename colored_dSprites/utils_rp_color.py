...
def from_latent_vector_2_color_para(code_input_raw):
...
def from_color_para_2_latent_vector(affine_color_para):
...
def affine_color_regularzier(real_code, trans_code):

    real_code_affine = real_code[:,:4] # first 4 code: theta, p, x, y
    trans_code_affine = trans_code[:,:4]

    real_code_color = real_code[:,4:]
    trans_code_color = trans_code[:,4:]
    ...
    # get color parameter from latent vector

    real_para_color = from_latent_vector_2_color_para(real_code_color)
    trans_para_color = from_latent_vector_2_color_para(trans_code_color)

    relative_color = trans_para_color/real_para_color

    code_rec_color = from_color_para_2_latent_vector(relative_color)
