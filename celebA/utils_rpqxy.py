...
def from_affine_para_2_latent_vector(affine_color_para):
    ...
    code_rec[:,2] = (affine_color_para[:,2] - 1)/pq_factor #q
    code_rec[:,3] = affine_color_para[:,3]/ xy_factor #x
    code_rec[:,4] = affine_color_para[:,4]/ xy_factor #y
...
def affine_regularzier(real_code, trans_code):
    ...
    real_code_affine = real_code[:,:5] 
    trans_code_affine = trans_code[:,:5]
    ...
    rec_theta_1 = relative_matrix[:,0,0] * relative_matrix[:,1,0] - relative_matrix[:,0,1] * relative_matrix[:,1,1]
    rec_theta_2 = relative_matrix[:,0,0]**2 + relative_matrix[:,1,1]**2 - relative_matrix[:,0,1]**2 - relative_matrix[:,1,0]**2
    rec_theta_3 = 2*rec_theta_1/rec_theta_2
    rec_theta = 0.5*torch.atan(rec_theta_3)


    rec_p =  relative_matrix[:,0,0] * torch.cos(rec_theta) + relative_matrix[:,1,0] * torch.sin(rec_theta)
    rec_q = -relative_matrix[:,0,1] * torch.sin(rec_theta) + relative_matrix[:,1,1] * torch.cos(rec_theta)


    rec_x = (relative_matrix[:,0,2]* torch.cos(rec_theta) + relative_matrix[:,1,2] * torch.sin(rec_theta))/rec_p
    rec_y = (relative_matrix[:,1,2]* torch.cos(rec_theta) - relative_matrix[:,0,2] * torch.sin(rec_theta))/rec_q

    # normalize the affine parameter back to latent vector

    affine_para = torch.stack((rec_theta, rec_p, rec_q, rec_x, rec_y), dim = 1)
